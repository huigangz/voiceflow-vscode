...
VoiceFlow 用一个小型本地程序(`voiceflow-mic.exe`)采集麦克风。Windows 11 的 **Smart App Control(SAC)** 会拦截**未签名**程序,而本 preview 的录音组件**尚未代码签名**。若你的机器开启了 SAC,按 `Ctrl+Alt+L` 会提示"录音组件被智能应用控制拦截"。

- **判断是否开启**:设置 → 隐私和安全性 → Windows 安全中心 → 应用和浏览器控制 → 智能应用控制。
- **本 preview 的应对**:这是**已知限制**。SAC 一旦关闭无法在不重装系统的情况下重新开启,因此**不建议为此关闭 SAC**。若你在受控/企业环境,请等待后续**已签名**版本,或在未开启 SAC 的机器上使用。
- whisper 转写二进制来自 whisper.cpp 官方 release、具备信誉,不受影响;仅本地采集组件受此限制。
- 正式发布前将对录音组件代码签名以消除此限制。
